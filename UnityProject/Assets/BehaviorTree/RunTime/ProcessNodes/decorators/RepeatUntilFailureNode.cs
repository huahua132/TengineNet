...
{
    [BehaviorProcessNode("RepeatUntilFailure",
    "+ 只能有一个子节点，多个仅执行第一个\n+ 只有当子节点返回失败时，才返回成功，其它情况返回运行中状态\n+ 如果设定了尝试次数，超过指定次数则返回失败",
    BehaviorProcessType.decorator)]
    public class RepeatUntilFailureNode : BehaviorProcessNodeBase
    {
        ...
        public override BehaviorRet OnTickRun()
        {
            ...
            {
                Debug.LogWarning("RepeatUntilFailure Node has no children");
            ...
            // 其它情况（SUCCESS或RUNNING）返回挂起，继续等待下次尝试
            return _Node.Yield();
        }
    }
}
